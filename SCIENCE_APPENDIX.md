...
The current production deployment `dpl_71gweLQDpDLEtSB1XgpuNyXM8v9P` was re-read in a fresh Chrome tab on 2026-08-27 with `?claim-check=58fd6f8`: `readyState=complete`, `appShell=true`, `bootState=false`, and `appLength=10,717`; the page served versioned assets `/src/main.js?v=claim-boundary-20260827` and `/src/styles.css?v=boot-state-20260827`, reported `WebMCP preview` and five tools live, displayed `Synthetic demo collection`, and had no horizontal overflow at 390×844, 768×1024, 1366×768, 1440×900, 1920×1080, or 2560×1440. The changed state was captured and visually inspected at all six sizes; the new footer remains a single readable line on desktop and follows the existing stacked footer treatment on mobile. On the same deployment, the human UI save path produced a visible approval request with the save action disabled while pending, then produced `Plan saved` after `Approve & save`. This is current visual and page-local state evidence, not native WebMCP discovery.
...
- Production deployment: `dpl_71gweLQDpDLEtSB1XgpuNyXM8v9P`, reported READY by Vercel; stable alias: [sidequest-webmcp.vercel.app](https://sidequest-webmcp.vercel.app).
