...
The current production deployment `dpl_9jZChZY2BekmU3Qk1XY9b4ukZWza` was re-read in a fresh Chrome tab on 2026-08-27 with `?check=3cf645e`: `readyState=complete`, `appShell=true`, `bootState=false`, and `appLength=10,715`; the page served versioned assets `/src/main.js?v=boot-state-20260827` and `/src/styles.css?v=boot-state-20260827`, reported `WebMCP preview` and five tools live, and emitted no captured error-level logs. At 390×844, 1366×768, and 2560×1440, `scrollWidth` equalled `clientWidth`. The same tab completed the human UI save path: a visible approval request appeared, the save action was disabled while pending, and “Approve & save” produced `Plan saved`. This is current visual and page-local state evidence, not native WebMCP discovery.
...
- Evidence-packet checklist commit: `a566caf2adf88a7aa3cf5f8d42277e02960ef9f2` (documentation-only human-gate checklist, including reproducible inputs and the corrected compatible swap example; pushed to `main`).
...
- Production deployment: `dpl_9jZChZY2BekmU3Qk1XY9b4ukZWza`, reported READY by Vercel; stable alias: [sidequest-webmcp.vercel.app](https://sidequest-webmcp.vercel.app).
