...
| An agent can prepare and tune a plan while a person controls saving. | Engineering/safety claim | Read-only annotations mark search/inspect; `save_plan` first creates a visible, current-plan-bound approval request, and saving requires both page-owned approval and `confirm === true`. Covered by `tests/webmcp.test.mjs`; no external action exists. |
...
Saving uses a fail-closed rule: `save_allowed = (plan exists) ∧ (saveRequest exists) ∧ (saveRequest.planId = plan.id) ∧ (saveApproval = true) ∧ (confirm === true)`. The page clears approval whenever the reviewed route changes, and the visible human approval is required before the agent-side confirmed call. The policy treats an accidental save as a higher-cost error than one extra confirmation request. No probability or user-harm estimate is claimed; the threshold is a deliberate product safety rule.
...
Open `http://localhost:4173` in ChatGPT’s in-app browser, or in Chrome 149+ after enabling `chrome://flags/#enable-webmcp-testing`. Inspect the page in Chrome DevTools’ WebMCP panel. Verify the five tools, then exercise: inspect → search → draft → swap → save request → human page approval → confirmed save → reset. Capture screenshots at widths 390, 768, 1366, 1440, 1920, and 2560 pixels and check that `document.documentElement.scrollWidth <= document.documentElement.clientWidth`.

The local Playwright path could not run because its browser cache and Chrome/Chromium executables were unavailable, and network installation was intentionally not used. A separate controlled Chrome session completed the six-width matrix documented below. A bounded in-app-browser attempt on 2026-08-27 first connected and loaded an older cached HTML shell with HTTP 200 and `readyState=complete`, but `#app` remained empty and the captured screenshot was blank. The stale document had no stylesheet link, no startup diagnostic, and an unversioned `/src/main.js` entrypoint. After one same-URL reload, the tab received the current versioned HTML, hydrated successfully (`data-app-ready=true`, non-empty `#app`), and reported `WebMCP ready` and `5 tools live`; captured logs remained empty. Native tool inspection still returned `gpt-5.6-luna does not support command "webmcp_list_tools"`, so this proves recovery and page-local native status, not host-side tool discovery or invocation. The separate Chrome result and the resolved source causes are recorded below.
...
The first blank page had two sequential module-graph causes. The static entrypoint imported `./styles.css` from `src/main.js`, which is invalid when the project is served as unbundled browser modules; the browser must receive CSS through an HTML stylesheet link. After that was corrected, Chrome exposed a second error: `main.js` imported `searchStops` from `data.js`, although `searchStops` is exported by `logic.js`. The final fix links `/src/styles.css` in `index.html`, removes the CSS import, wires the named exports to their owning modules, and adds static build guards for both invariants. The HTML shell now also contains a visible boot status outside the `#app` mount; successful rendering sets `data-app-ready="true"` and hides it, so a module or host execution failure remains diagnosable rather than silently white. Versioned module and stylesheet URLs (`?v=startup-fallback-20260827`) ensure the browser does not reuse pre-fix assets.

After the historical repaired-load deployment `dpl_G5acXESgUg4W2uw7U8ZvnN5GKiSr`, Chrome loaded the cache-busted entrypoint with the stylesheet, produced a non-empty `#app` (`10,712` HTML characters), rendered the route and handoff panels, and had no new page error during the repaired load. Clicking “Preview an agent handoff” updated the visible activity rail with inspect/search/draft events. This verifies the visual/runtime path; it does not verify native WebMCP discovery.
...
The current production deployment `dpl_9hSkJTtu3Xob96rfpiAK81ubJeRU` was re-read in the in-app browser on 2026-08-27 after a same-URL reload: `readyState=complete`, `appShell=true`, `data-app-ready=true`, `startupVisible=false`, and `appLength=10,714`; the page served versioned assets `/src/main.js?v=startup-fallback-20260827` and `/src/styles.css?v=startup-fallback-20260827`, reported `WebMCP ready` and five tools live, and had no horizontal overflow in the earlier six-width Chrome matrix at 390×844, 768×1024, 1366×768, 1440×900, 1920×1080, or 2560×1440. The deployment route table also contains `Cache-Control: no-store` and `Vercel-CDN-Cache-Control: no-store` for `/` and `/index.html`. The local contract suite includes a native-registration rejection case, and it passed; the available browser wrapper still could not invoke native `webmcp_list_tools`, so no host-side tool discovery or invocation claim is made. A bounded JavaScript-disabled fault-path check on the prior runtime deployment already established that the sibling startup status remains visible when hydration cannot run. This is current visual and page-local state evidence, not native WebMCP discovery.
...
| A caller tries to make a state-changing save look read-only or bypass consent with a truthy string. | Only an explicitly human-approved mutation for the current plan may add a saved plan. | Read-only annotations are limited to search/inspect in `src/webmcp.js`; `save_plan` creates a page-visible request, requires `values.confirm === true`, and requires `saveApproval` for the same plan id before calling `store.savePlan`; the tests cover false, string, premature true, approval, and approval invalidation after a route change. | The browser host's own confirmation UX was not available in this worktree; native live invocation remains open. |
...
- Production deployment: `dpl_9hSkJTtu3Xob96rfpiAK81ubJeRU`, reported READY by Vercel; stable alias: [sidequest-webmcp.vercel.app](https://sidequest-webmcp.vercel.app). The application/runtime revision remains `1f9caeda290c1dee738b35dd1e0df60516bf9397`; source/config revision deployed for the cache policy is `5c8e6cb0a7cd2224bcaa7e6fa09c128ae7f595ba`.
...
- Browser boundary: the in-app browser's first load was blank because it served a stale unversioned HTML shell; one reload recovered the current versioned shell and page-local `WebMCP ready` status. The browser wrapper still cannot run native `webmcp_list_tools`, so no host-side tool discovery or invocation is claimed. If a fresh human browser remains blank after one reload, use the Chrome 149+ fallback in `docs/human-gate-checklist.md`.
