...
The current production deployment `dpl_2RMYh2TAvgTiaYDiF8P3iuoczn9r` was re-read in a fresh Chrome tab on 2026-08-27 with `?v=71a2618`: `readyState=complete`, `appShell=true`, `bootState=false`, and `appLength=10,715`; the page reported `WebMCP preview` and five tools live. At 390×844, 1366×768, and 2560×1440, `scrollWidth` equalled `clientWidth`. The same tab completed the human UI save path: a visible approval request appeared, the save action was disabled while pending, and “Approve & save” produced `Plan saved`. This is current visual and page-local state evidence, not native WebMCP discovery.
...
- Application/runtime revision: `71a26189dcdcbadcf935531fe840f9b144933a12` (page-owned save approval plus visible static boot state; deployed below).
- Production deployment: `dpl_2RMYh2TAvgTiaYDiF8P3iuoczn9r`, reported READY by Vercel; stable alias: [sidequest-webmcp.vercel.app](https://sidequest-webmcp.vercel.app).
