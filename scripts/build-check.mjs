...
const required = ['index.html', 'LICENSE', 'README.md', 'SCIENCE_APPENDIX.md', 'docs/spec.md', 'docs/devpost-form-answers.md', 'docs/human-gate-checklist.md', 'scripts/smoke.mjs', 'src/main.js', 'src/styles.css', 'src/webmcp.js', 'src/store.js', 'src/logic.js', 'src/data.js'];
...
const startupIndex = html.indexOf('<div id="startup-status"');
const appIndex = html.indexOf('<div id="app"></div>');
if (startupIndex < 0 || appIndex < 0 || startupIndex > appIndex || !html.includes('class="boot-state"') || !styles.includes('.boot-state') || !styles.includes('#startup-status')) throw new Error('missing static boot state outside the app mount');
