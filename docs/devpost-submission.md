...
The zero-dependency `npm run smoke` receipt exercises `inspect_plan → search_stops → draft_plan → inspect_plan → swap_stop → save_plan(false) → human_approve_save → save_plan(true) → reset`. On the checked revision it returned two constrained coffee candidates, a three-stop route of 85 minutes and $8, preserved the two untouched stops during the swap, denied both the initial and premature save attempts with zero saved plans, accepted the confirmed call only after page-owned approval, and reset to a blank state. `npm run check` passes 16 tests and the static build gate for 14 required files, five WebMCP tools, and accessibility/security guards.
